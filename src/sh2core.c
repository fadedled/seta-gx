...
int SH2AddMemoryBreakpoint(SH2_struct *context, u32 addr, u32 flags) {
   ...
   if (context->bp.nummemorybreakpoints < MAX_BREAKPOINTS) {
      ...
      context->bp.memorybreakpoint[context->bp.nummemorybreakpoints].oldreadbyte = mem_read8_arr[(addr >> 16) & 0xFFF];
      context->bp.memorybreakpoint[context->bp.nummemorybreakpoints].oldreadword = mem_read16_arr[(addr >> 16) & 0xFFF];
      context->bp.memorybreakpoint[context->bp.nummemorybreakpoints].oldreadlong = mem_read32_arr[(addr >> 16) & 0xFFF];
      context->bp.memorybreakpoint[context->bp.nummemorybreakpoints].oldwritebyte = mem_write8_arr[(addr >> 16) & 0xFFF];
      context->bp.memorybreakpoint[context->bp.nummemorybreakpoints].oldwriteword = mem_write16_arr[(addr >> 16) & 0xFFF];
      context->bp.memorybreakpoint[context->bp.nummemorybreakpoints].oldwritelong = mem_write32_arr[(addr >> 16) & 0xFFF];
      ...
      if (flags & BREAK_BYTEREAD)
      {
         ...
         if (!CheckForMemoryBreakpointDupes(context, addr, BREAK_BYTEREAD, &which))
            mem_read8_arr[(addr >> 16) & 0xFFF] = &SH2MemoryBreakpointReadByte;
         else
      ...
      if (flags & BREAK_WORDREAD)
      {
         ...
         if (!CheckForMemoryBreakpointDupes(context, addr, BREAK_WORDREAD, &which))
            mem_read16_arr[(addr >> 16) & 0xFFF] = &SH2MemoryBreakpointReadWord;
         else
            // fix old memory access function
      ...
      if (flags & BREAK_LONGREAD)
      {
         ...
         if (!CheckForMemoryBreakpointDupes(context, addr, BREAK_LONGREAD, &which))
            mem_read32_arr[(addr >> 16) & 0xFFF] = &SH2MemoryBreakpointReadLong;
         else
            // fix old memory access function
            context->bp.memorybreakpoint[context->bp.nummemorybreakpoints].oldreadword = context->bp.memorybreakpoint[which].oldreadword;
      }
      ...
      if (flags & BREAK_BYTEWRITE)
      {
         ...
         if (!CheckForMemoryBreakpointDupes(context, addr, BREAK_BYTEWRITE, &which))
            mem_write8_arr[(addr >> 16) & 0xFFF] = &SH2MemoryBreakpointWriteByte;
         else
            // fix old memory access function
      ...
      if (flags & BREAK_WORDWRITE)
      {
         ...
         if (!CheckForMemoryBreakpointDupes(context, addr, BREAK_WORDWRITE, &which))
            mem_write16_arr[(addr >> 16) & 0xFFF] = &SH2MemoryBreakpointWriteWord;
         else
            // fix old memory access function
      ...
      if (flags & BREAK_LONGWRITE)
      {
         ...
         if (!CheckForMemoryBreakpointDupes(context, addr, BREAK_LONGWRITE, &which))
            mem_write32_arr[(addr >> 16) & 0xFFF] = &SH2MemoryBreakpointWriteLong;
         else
            // fix old memory access function
      ...
      for (i = 0; i < context->bp.nummemorybreakpoints; i++) {
         if (context->bp.memorybreakpoint[i].addr == addr)
         {
            ...
            if (context->bp.memorybreakpoint[i].flags & BREAK_BYTEREAD)
               mem_read8_arr[(addr >> 16) & 0xFFF] = context->bp.memorybreakpoint[i].oldreadbyte;

            if (context->bp.memorybreakpoint[i].flags & BREAK_WORDREAD)
               mem_read16_arr[(addr >> 16) & 0xFFF] = context->bp.memorybreakpoint[i].oldreadword;

            if (context->bp.memorybreakpoint[i].flags & BREAK_LONGREAD)
               mem_read32_arr[(addr >> 16) & 0xFFF] = context->bp.memorybreakpoint[i].oldreadlong;

            if (context->bp.memorybreakpoint[i].flags & BREAK_BYTEWRITE)
               mem_write8_arr[(addr >> 16) & 0xFFF] = context->bp.memorybreakpoint[i].oldwritebyte;

            if (context->bp.memorybreakpoint[i].flags & BREAK_WORDWRITE)
               mem_write16_arr[(addr >> 16) & 0xFFF] = context->bp.memorybreakpoint[i].oldwriteword;

            if (context->bp.memorybreakpoint[i].flags & BREAK_LONGWRITE)
               mem_write32_arr[(addr >> 16) & 0xFFF] = context->bp.memorybreakpoint[i].oldwritelong;
