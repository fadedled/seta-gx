...
#else
static int SCSScsp2Init(int coreid, void (*interrupt_handler)(void));
static void SCSScsp2DeInit(void);
static void SCSScsp2Reset(void);
static int SCSScsp2ChangeVideoFormat(int type);
static void SCSScsp2Exec(int decilines);
static void SCSScsp2MuteAudio(int flags);
static void SCSScsp2UnMuteAudio(int flags);
static void SCSScsp2SetVolume(int volume);
static void SCSScsp2M68KStart(void);
static void SCSScsp2M68KStop(void);
static u8 FASTCALL SCSScsp2SoundRamReadByte(u32 addr);
static void FASTCALL SCSScsp2SoundRamWriteByte(u32 addr, u8 val);
static u16 FASTCALL SCSScsp2SoundRamReadWord(u32 addr);
static void FASTCALL SCSScsp2SoundRamWriteWord(u32 addr, u16 val);
static u32 FASTCALL SCSScsp2SoundRamReadLong(u32 addr);
static void FASTCALL SCSScsp2SoundRamWriteLong(u32 addr, u32 val);
static void SCSScsp2ScspReceiveCDDA(const u8 *sector);
static int SCSSScsp2SoundSaveState(FILE *fp);
static int SCSSScsp2SoundLoadState(FILE *fp, int version, int size);
static u32 FASTCALL SCSScsp2M68KReadWord(u32 address);
static u8 FASTCALL SCSScsp2ReadByte(u32 address);
static u16 FASTCALL SCSScsp2ReadWord(u32 address);
static u32 FASTCALL SCSScsp2ReadLong(u32 address);
static void FASTCALL SCSScsp2WriteByte(u32 address, u8 data);
static void FASTCALL SCSScsp2WriteWord(u32 address, u16 data);
static void FASTCALL SCSScsp2WriteLong(u32 address, u32 data);
static M68KBreakpointInfo *SCSScsp2M68KGetBreakpointList(void);

SCSPInterface_struct SCSScsp2 = {
SCSCORE_SCSP2,
"Scsp2 SCSP Interface",
SCSScsp2Init,
SCSScsp2DeInit,
SCSScsp2Reset,
SCSScsp2ChangeVideoFormat,
SCSScsp2Exec,
SCSScsp2MuteAudio,
SCSScsp2UnMuteAudio,
SCSScsp2SetVolume,
SCSScsp2M68KStart,
SCSScsp2M68KStop,
SCSScsp2SoundRamReadByte,
SCSScsp2SoundRamWriteByte,
SCSScsp2SoundRamReadWord,
SCSScsp2SoundRamWriteWord,
SCSScsp2SoundRamReadLong,
SCSScsp2SoundRamWriteLong,
SCSScsp2ScspReceiveCDDA,
SCSSScsp2SoundSaveState,
SCSSScsp2SoundLoadState,
SCSScsp2M68KReadWord,
SCSScsp2ReadByte,
SCSScsp2ReadWord,
SCSScsp2ReadLong,
SCSScsp2WriteByte,
...
#define SCSP_CLOCK_INC_NTSC     ((u64)((double)((u64)SCSP_CLOCK_FREQ<<20) / 60.0 / 263.0))
#define SCSP_CLOCK_INC_PAL	((u64)((double)((u64)SCSP_CLOCK_FREQ<<20) / 50.0 / 313.0))
#endif
...
static s32 scsp_buffer[SCSP_SOUND_BUFSIZE << 1];
...
static s32 *scsp_buf;            // Base pointer for left channel
   ...
   {                                                                        \
      ...
      {                                                                     \
         ...
         {                                                                  \
            ...
            out *= env;                                                     \
            if (R)                                                          \
               scsp_buf[pos] += out >> slot->outshift_r;                    \
            if (L)                                                          \
               scsp_buf[pos+1] += out >> slot->outshift_l;                  \
         }                                                                  \
      }                                                                     \
                                                                            \
...
#else
int SCSScsp2Init(int coreid, void (*interrupt_handler)(void))
#endif
{
   int i, j;
   double x;
...
#else
      x = pow(((double) (SCSP_ENV_MASK - i) / (double)SCSP_ENV_LEN), 4.0);
#endif
...
#else
      scsp_tl_table[i] = round(pow(2.0, -((double)i/16.0)) * (double)(1 << SCSP_TL_BITS));
#endif
...
#ifdef SCSP_PLUGIN
//-------------------------------------------------------------------------
// ScspChangeCore:  Change the module used for scsp.  Returns
// 0 on success, -1 on error.

int ScspChangeCore(int coreid)
{
   int i;
   ...
   if (coreid == SCSCORE_DEFAULT)
      SCSCore = SCSCoreList[0];
   else
   {
      // Otherwise, go through core list and find the id
      for (i = 0; SCSCoreList[i] != NULL; i++)
      {
         if (SCSCoreList[i]->id == coreid)
         {
            // Set to current core
            SCSCore = SCSCoreList[i];
            break;
         }
      }
   }
...
#else
int SCSScsp2ChangeVideoFormat(int type)
#endif
{
   scsp_clock_inc = yabsys.IsPal ? SCSP_CLOCK_INC_PAL : SCSP_CLOCK_INC_NTSC;
...
#else
void SCSScsp2MuteAudio(int flags)
#endif
{
...
#else
void SCSScsp2UnMuteAudio(int flags)
#endif
{
   scsp_mute_flags &= ~flags;
...
#else
void SCSScsp2SetVolume(int volume)
#endif
{
   scsp_volume = volume;
   if (SNDCore)
...
#else
void SCSScsp2Exec(int decilines)
#endif
{
   u32 new_target;
...
#else
   scsp_clock_frac += scsp_clock_inc * decilines / declinenum;
#endif
...
static void ScspThread(void)
{
   ...
   while (PSP_UC(scsp_thread_running))
   {
      ...
      if (clock_cycles > 0)
      {
         ...
         YabThreadSleep();
#else
         YabThreadSleep(YAB_THREAD_SCSP);
#endif
   }
#endif
}
...
static void ScspDoExec(u32 cycles)
{
   ...
   while (cycles_left > 0)
   {
      ...
      while (sample_count > 0)
      {
         ...
         buf = &scsp_buffer[scsp_sound_genpos << 1];
         ScspGenerateAudio(buf, NULL, this_count);
...
static void ScspGenerateAudio(s32 *bufL, s32 *bufR, u32 samples)
{
   ...
   memset(bufL, 0, sizeof(*bufL) * (samples << 1));
   ...
   if (cdda_next_out != PSP_UC(cdda_next_in) * 2352)
   {
      ...
      if (cdda_delay == 0)
         ScspGenerateAudioForCDDA(bufL, NULL, samples);
   }
   if (cdda_next_out == PSP_UC(cdda_next_in) * 2352)
...
#else
u8 FASTCALL SCSScsp2SoundRamReadByte(u32 address)
#endif
{
...
#else
u16 FASTCALL SCSScsp2SoundRamReadWord(u32 address)
#endif
{
   address &= scsp.sound_ram_mask;
...
#else
u32 FASTCALL SCSScsp2SoundRamReadLong(u32 address)
#endif
{
   address &= scsp.sound_ram_mask;
...
#else
void FASTCALL SCSScsp2SoundRamWriteByte(u32 address, u8 data)
#endif
{
   address &= scsp.sound_ram_mask;
...
#else
void FASTCALL SCSScsp2SoundRamWriteWord(u32 address, u16 data)
#endif
{
   address &= scsp.sound_ram_mask;
...
#else
void FASTCALL SCSScsp2SoundRamWriteLong(u32 address, u32 data)
#endif
{
   address &= scsp.sound_ram_mask;
...
#else
u8 FASTCALL SCSScsp2ReadByte(u32 address)
#endif
{
#ifndef SCSP_PLUGIN
   const u16 data = ScspReadWord(address & ~1);
#else
   const u16 data = SCSScsp2ReadWord(address & ~1);
#endif
   if (address & 1)
      return data & 0xFF;
   else
...
#else
u16 FASTCALL SCSScsp2ReadWord(u32 address)
#endif
{
...
#else
u32 FASTCALL SCSScsp2ReadLong(u32 address)
#endif
{
#ifndef SCSP_PLUGIN
   return (u32)ScspReadWord(address) << 16 | ScspReadWord(address+2);
#else
   return (u32)SCSScsp2ReadWord(address) << 16 | SCSScsp2ReadWord(address+2);
#endif
}
...
#else
void FASTCALL SCSScsp2WriteByte(u32 address, u8 data)
#endif
{
#if 0
   if (scsp_thread_running)
...
#else
void FASTCALL SCSScsp2WriteWord(u32 address, u16 data)
#endif
{
#if 0
   if (scsp_thread_running)
...
#else
void FASTCALL SCSScsp2WriteLong(u32 address, u32 data)
#endif
{
#if 0
   if (scsp_thread_running)
...
#else
void SCSScsp2ScspReceiveCDDA(const u8 *sector)
#endif
{
...
#else
int SCSSScsp2SoundSaveState(FILE *fp)
#endif
{
   int i;
   u32 temp;
...
#else
int SCSSScsp2SoundLoadState(FILE *fp, int version, int size)
#endif
{
   int i, i2;
   u32 temp;
...
#else
void Scsp2Convert32uto16s(s32 *srcL, s32 *srcR, s16 *dest, u32 len)
#endif
{
   u32 i;
...
static u16 ScspReadMonitor(void)
{
	u8 ca = (scsp.slot[scsp.mslc].addr_counter >> (SCSP_FREQ_LOW_BITS + 12)) & 0xF;
	u8 sgc = 3 - scsp.slot[scsp.mslc].env_phase;
	u8 eg = 0x1f - (scsp.slot[scsp.mslc].last_env >> 27);
...
#else
M68KBreakpointInfo *SCSScsp2M68KGetBreakpointList(void)
#endif
{
...
#else
u32 FASTCALL SCSScsp2M68KReadWord(u32 address)
#endif
{
   if (address < 0x100000)
