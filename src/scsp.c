...
#ifdef SCSP_PLUGIN
static int SCSScsp1Init(int coreid, void (*interrupt_handler)(void));
static void SCSScsp1DeInit(void);
static void SCSScsp1Reset(void);
static int SCSScsp1ChangeVideoFormat(int type);
static void SCSScsp1Exec(int decilines);
static void SCSScsp1MuteAudio(int flags);
static void SCSScsp1UnMuteAudio(int flags);
static void SCSScsp1SetVolume(int volume);
static void SCSScsp1M68KStart(void);
static void SCSScsp1M68KStop(void);
static u8 FASTCALL SCSScsp1SoundRamReadByte(u32 addr);
static void FASTCALL SCSScsp1SoundRamWriteByte(u32 addr, u8 val);
static u16 FASTCALL SCSScsp1SoundRamReadWord(u32 addr);
static void FASTCALL SCSScsp1SoundRamWriteWord(u32 addr, u16 val);
static u32 FASTCALL SCSScsp1SoundRamReadLong(u32 addr);
static void FASTCALL SCSScsp1SoundRamWriteLong(u32 addr, u32 val);
static void SCSScsp1ScspReceiveCDDA(const u8 *sector);
static int SCSScsp1SoundSaveState(FILE *fp);
static int SCSScsp1SoundLoadState(FILE *fp, int version, int size);
static u32 FASTCALL SCSScsp1M68KReadWord(const u32 adr);
static u8 FASTCALL SCSScsp1ReadByte(u32 address);
static u16 FASTCALL SCSScsp1ReadWord(u32 address);
static u32 FASTCALL SCSScsp1ReadLong(u32 address);
static void FASTCALL SCSScsp1WriteByte(u32 address, u8 data);
static void FASTCALL SCSScsp1WriteWord(u32 address, u16 data);
static void FASTCALL SCSScsp1WriteLong(u32 address, u32 data);
static m68kcodebreakpoint_struct *SCSScsp1M68KGetBreakpointList();

SCSPInterface_struct SCSScsp1 = {
SCSCORE_SCSP1,
"Scsp1 SCSP Interface",
SCSScsp1Init,
SCSScsp1DeInit,
SCSScsp1Reset,
SCSScsp1ChangeVideoFormat,
SCSScsp1Exec,
SCSScsp1MuteAudio,
SCSScsp1UnMuteAudio,
SCSScsp1SetVolume,
SCSScsp1M68KStart,
SCSScsp1M68KStop,
SCSScsp1SoundRamReadByte,
SCSScsp1SoundRamWriteByte,
SCSScsp1SoundRamReadWord,
SCSScsp1SoundRamWriteWord,
SCSScsp1SoundRamReadLong,
SCSScsp1SoundRamWriteLong,
SCSScsp1ScspReceiveCDDA,
SCSScsp1SoundSaveState,
SCSScsp1SoundLoadState,
SCSScsp1M68KReadWord,
SCSScsp1ReadByte,
SCSScsp1ReadWord,
SCSScsp1ReadLong,
SCSScsp1WriteByte,
...
#else
SCSScsp1M68KReadWord (const u32 adr)
#endif
{
  if (adr < 0x100000)
...
#else
SCSScsp1SoundRamReadByte (u32 addr)
#endif
{
  addr &= 0xFFFFF;
...
#else
SCSScsp1SoundRamWriteByte (u32 addr, u8 val)
#endif
{
  addr &= 0xFFFFF;
...
#else
SCSScsp1SoundRamReadWord (u32 addr)
#endif
{
  addr &= 0xFFFFF;
...
#else
SCSScsp1SoundRamWriteWord (u32 addr, u16 val)
#endif
{
  addr &= 0xFFFFF;
...
#else
SCSScsp1SoundRamReadLong (u32 addr)
#endif
{
  addr &= 0xFFFFF;
...
#else
SCSScsp1SoundRamWriteLong (u32 addr, u32 val)
#endif
{
  addr &= 0xFFFFF;
...
ScspInit (int coreid)
#else
SCSScsp1Init(int coreid, void (*interrupt_handler)(void))
#endif
{
  int i;
...
#else
SCSScsp1ChangeVideoFormat (int type)
#endif
{
...
#else
SCSScsp1ScspReceiveCDDA (const u8 *sector)
#endif
{
...
#else
Scsp1M68KSetBreakpointCallBack (void (*func)(u32))
#endif
{
...
#else
Scsp1M68KAddCodeBreakpoint (u32 addr)
#endif
{
  int i;
...
#else
Scsp1M68KDelCodeBreakpoint (u32 addr)
#endif
{
  int i;
...
#else
SCSScsp1SoundSaveState (FILE *fp)
#endif
{
  int i;
  u32 temp;
...
#else
SCSScsp1SoundLoadState (FILE *fp, int version, int size)
#endif
{
  int i, i2;
  u32 temp;
