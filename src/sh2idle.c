...
void FASTCALL SH2idleCheck(SH2_struct *context, u32 cycles) {
  ...
  for (;;) {
	...
	case 11: //SH2bf
	  ...
	  goto branching_reached;
	  break;
	default: decode(context->instruction)(context);//opcodes[context->instruction](context);
	}
	} else decode(context->instruction)(context);//opcodes[context->instruction](context);
...
#ifdef IDLE_DETECT_VERBOSE
  {
    ...
    if (( !~bDet )&&(loopBegin!=oldLoopBegin[context==MSH2][0])&&(loopBegin!=oldLoopBegin[context==MSH2][1])) {
      ...
      for ( ; offset <= end ; offset+=2 ) {
        SH2Disasm(offset, mem_Read16(offset), 0, lineBuf);
        printf( "%s\n", lineBuf );
      }
